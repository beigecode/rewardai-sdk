...
export class RewardAI {
    ...
    async distribute(params) {
        ...
        if (!this.keypair) {
            throw new Error('Keypair required for real transfers. Pass keypair in config: new RewardAI({ keypair })');
        }
        if (!this.connection) {
        ...
        for (const recipient of recipients) {
            try {
                ...
                const sourceTokenAccount = await getAssociatedTokenAddress(tokenMintPubkey, sourceWalletPubkey);
                // Get or create destination token account
                const destinationTokenAccount = await getOrCreateAssociatedTokenAccount(this.connection, this.keypair, tokenMintPubkey, recipientPubkey);
                // Create transfer instruction
                const transferInstruction = createTransferInstruction(sourceTokenAccount, destinationTokenAccount.address, sourceWalletPubkey, amountToSend, [], TOKEN_PROGRAM_ID);
                // Create and send transaction
                const transaction = new Transaction().add(transferInstruction);
                const signature = await this.connection.sendTransaction(transaction, [this.keypair], { skipPreflight: false, preflightCommitment: 'confirmed' });
                // Confirm transaction
